...
class BrainConfig:
    ...
    def __init__(self):
        ...
        self.TEMP_DIR = getattr(
            settings,
            "BRAIN_TEMP_DIR",
            self.BASE_DIR / "media" / "brain" / "temp_extracts",
        )
        self.OUTPUT_DIR = getattr(
            settings,
            "BRAIN_OUTPUT_DIR",
            self.BASE_DIR / "media" / "brain" / "qa_outputs",
        )
        self.UPLOADS_DIR = getattr(
            settings, "BRAIN_UPLOADS_DIR", self.BASE_DIR / "media" / "brain" / "uploads"
        )
        ...
        self.RATE_LIMIT_BATCH_SIZE = self.config.get(
            "RATE_LIMIT_BATCH_SIZE", 3
        )  # Number of chunks to process before cooling down
        self.RATE_LIMIT_COOLDOWN = self.config.get(
            "RATE_LIMIT_COOLDOWN", 10
        )  # seconds between batches

        # Model settings
        self.DEFAULT_GEMINI_MODEL = self.config.get(
            "DEFAULT_GEMINI_MODEL", "models/gemini-1.5-flash"
        )
        self.QA_GEMINI_MODEL = self.config.get(
            "QA_GEMINI_MODEL", "models/gemini-1.5-flash"
        )
        self.FALLBACK_GEMINI_MODEL = self.config.get(
        ...
        self.QUESTION_TYPE = self.config.get(
            "QUESTION_TYPE", "MULTIPLECHOICE"
        )  # Options: "SHORT" or "MULTIPLECHOICE"
        self.ANSWER_OPTIONS = self.config.get(
            "ANSWER_OPTIONS", 4
