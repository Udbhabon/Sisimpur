...
class PromptManager:
    ...
    def get_prompt(
        ...
        try:
            ...
            logger.info(
                f"Generated prompt for: {language}/{document_type}/{question_type}/{question_count_mode}"
            )
            ...
            return self._get_fallback_prompt(
                text, num_questions or 5, question_type, language
            )
    ...
    def _get_prompt_from_module(
        self, language: str, doc_type: str, q_type: str, count_mode: str
    ) -> str:
        ...
        try:
            ...
            formatted = template.format(
                ...
                option_labels_en=(
                    "A, B, C, D"
                    if answer_options == 4
                    else ", ".join([chr(65 + i) for i in range(answer_options)])
                ),
                option_labels_bn=(
                    "ক, খ, গ, ঘ"
                    if answer_options == 4
                    else ", ".join(
                        ["ক", "খ", "গ", "ঘ", "ঙ", "চ", "ছ", "জ"][:answer_options]
                    )
                ),
            )
...
{{
  "questions": [
    ...
    def _get_fallback_prompt(
        self, text: str, num_questions: int, question_type: str, language: str
    ) -> str:
        """Get a simple fallback prompt"""
        template = self._get_basic_template(language, question_type.lower(), "specific")
        return template.format(text=text, num_questions=num_questions, answer_options=4)
