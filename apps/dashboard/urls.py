...
urlpatterns = [
    ...
    path(
        "flashcard/start/<int:job_id>/", views.start_flashcard, name="start_flashcard"
    ),
    path(
        "flashcard/session/<str:session_id>/",
        views.flashcard_session,
        name="flashcard_session",
    ),
    path(
        "flashcard/complete/<str:session_id>/",
        views.complete_flashcard,
        name="complete_flashcard",
    ),
    # API endpoints for AJAX (Document Processing Only)
    path(
        "api/process-document/", views.api_process_document, name="api_process_document"
    ),
