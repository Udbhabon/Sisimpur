...
def home(request):
    ...
    try:
        ...
        recent_jobs = ProcessingJob.objects.filter(user=request.user).order_by(
            "-created_at"
        )[:5]
    except:
        recent_jobs = []
...
def quiz_results(request, job_id):
    ...
    if job:
        ...
        form_settings["selected_language_display"] = dict(job.LANGUAGE_CHOICES).get(
            job.language, job.language
        )
        form_settings["selected_question_type_display"] = dict(
            job.QUESTION_TYPE_CHOICES
        ).get(job.question_type, job.question_type)
        ...
        detected_values["detected_language_display"] = {
            ...
            detected_values["detected_language"], detected_values["detected_language"]
        )
...
def api_job_status(request, job_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": f"Brain processing not available: {str(e)}"},
            status=500,
        )
...
def leaderboard(request):
    ...
    user_stats = (
        User.objects.annotate(
            total_exams=Count(
                "exam_sessions", filter=Q(exam_sessions__in=exam_sessions)
            ),
            total_score=Sum(
                "exam_sessions__total_score", filter=Q(exam_sessions__in=exam_sessions)
            ),
            avg_percentage=Avg(
                "exam_sessions__percentage_score",
                filter=Q(exam_sessions__in=exam_sessions),
            ),
            total_credit_points=Sum(
                "exam_sessions__credit_points",
                filter=Q(exam_sessions__in=exam_sessions),
            ),
        )
        .filter(total_exams__gt=0)  # Only users who have taken exams
    ...
    total_users = (
        User.objects.filter(exam_sessions__status="completed").distinct().count()
    )
    active_users_week = (
        User.objects.filter(
...
def start_exam(request, job_id):
    ...
    try:
        ...
        user_attempts = ExamSession.objects.filter(
            user=request.user, processing_job=job
        ).count()
        ...
        exam_session = ExamSession.objects.create(
            ...
            time_limit_minutes=len(questions_list)
            * config.default_time_per_question_minutes,
...
def exam_session(request, session_id):
    ...
    """
    print(
        f"DEBUG: exam_session called with session_id={session_id}, user={request.user}"
    )
    try:
        ...
        print(
            f"DEBUG: Found exam session: {exam_session}, status: {exam_session.status}"
        )

        # Check if session is still active
        if exam_session.status != "active":
            print(
                f"DEBUG: Session not active (status: {exam_session.status}), redirecting to exam_result"
            )
        ...
        print(
            f"DEBUG: Current question index: {current_index}, Total questions: {len(exam_session.questions_order)}"
        )
        ...
        existing_answer = ExamAnswer.objects.filter(
            exam_session=exam_session, question=current_question
        ).first()
        ...
        if request.method == "POST":
            ...
            if user_answer:
                ...
                if current_question.question_type == "MULTIPLECHOICE":
                    ...
                    is_correct = (
                        user_answer.lower().strip() in current_question.answer.lower()
                    )
            ...
            if (
                action == "next"
                and current_index < len(exam_session.questions_order) - 1
            ):
                ...
                action == "previous"
                and current_index > 0
                and exam_session.allow_navigation
            ):
                exam_session.current_question_index -= 1
                exam_session.save()
            elif action == "submit":
        ...
        progress_percentage = (
            (current_index + 1) / len(exam_session.questions_order)
        ) * 100
...
def exam_result(request, session_id):
    ...
    """
    print(
        f"DEBUG: exam_result called with session_id={session_id}, user={request.user}"
    )
    try:
        from .models import ExamSession, ExamAnswer
        ...
        answers = ExamAnswer.objects.filter(exam_session=exam_session).order_by(
            "question_index"
        )
        ...
        print(
            f"DEBUG: Stats - Total: {total_questions}, Answered: {answered_questions}, Correct: {correct_answers}, Incorrect: {incorrect_answers}"
        )
...
def flashcard_session(request, session_id):
    ...
    try:
        ...
        flashcard_session = get_object_or_404(
            FlashcardSession, session_id=session_id, user=request.user
        )
        ...
        if request.method == "POST":
            ...
            if flashcard_session.current_card_index >= len(
                flashcard_session.cards_order
            ):
                flashcard_session.status = "completed"
                flashcard_session.completed_at = timezone.now()
                flashcard_session.save()
        ...
        context = {
            ...
            "progress_percentage": (
                (current_index) / len(flashcard_session.cards_order)
            )
            * 100,
