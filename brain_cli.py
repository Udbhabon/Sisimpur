...
def process_document(
    file_path, num_questions=None, language="auto", question_type="MULTIPLECHOICE"
):
...
def list_jobs():
    ...
    print("-" * 80)
    print(
        f"{'ID':<5} {'Document':<30} {'Status':<12} {'Questions':<10} {'Created':<20}"
    )
    print("-" * 80)
    ...
    for job in jobs:
        ...
        print(
            f"{job.id:<5} {job.document_name[:29]:<30} {job.status:<12} {qa_count:<10} {job.created_at.strftime('%Y-%m-%d %H:%M'):<20}"
        )
...
def main():
    ...
    process_parser.add_argument(
        "-n", "--num-questions", type=int, help="Number of questions to generate"
    )
    process_parser.add_argument(
        "-l",
        "--language",
        default="auto",
        choices=["auto", "english", "bengali"],
    ...
    test_parser = subparsers.add_parser(
        "test", help="Run a quick test with sample data"
    )
