...
import { getFlashcardSession, advanceFlashcard } from "@/lib/api";
import { useToast } from "@/components/ui/ToastProvider";
import { cn } from "@/lib/utils";
import type { FlashcardSessionResponse } from "@/lib/types";
...
export default function FlashcardPage() {
  ...
  useEffect(() => {
    if (session?.status === "completed") {
      router.push(`/flashcard/complete/${sessionId}`);
    }
    ...
    <div className="max-w-xl mx-auto space-y-6 fade-in">
      ...
      {flipped && (
        <div className="flex gap-3 fade-in">
          ...
          <button
            onClick={async () => { await advanceFlashcard(sessionId, "skip"); await refresh(); }}
            disabled={advancing}
      ...
      <button
        onClick={() => router.push(`/flashcard/complete/${sessionId}`)}
