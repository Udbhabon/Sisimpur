...
import { getMyQuizzes, startExam, startFlashcard } from "@/lib/api";
import { useToast } from "@/components/ui/ToastProvider";
import { cn } from "@/lib/utils";
import type { ProcessingJob } from "@/lib/types";

function GiveExamContent() {
  ...
  async function handleStart() {
    ...
    try {
      if (mode === "exam") {
        ...
        router.push(`/flashcard/${res.session_id}`);
      }
    } catch (e: unknown) {
...
export default function GiveExamPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-64"><i className="ri-loader-4-line animate-spin text-3xl text-white/30" /></div>}>
      <GiveExamContent />
    </Suspense>
  );
}
