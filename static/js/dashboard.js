document.addEventListener("DOMContentLoaded", function () {
  ...
  languageOptions.forEach((option) => {
    option.addEventListener("click", (e) => {
      ...
      document.querySelector(".language-current span").textContent =
        langText.split(" ")[0];

      // Update active state
      document
        .querySelector(".language-option.active")
  ...
  const preventBubbling = document.querySelectorAll(
    ".language-dropdown, .profile-dropdown",
  );
  ...
  if (generateBtn) {
    generateBtn.addEventListener("click", () => {
      ...
      const activeDifficultyBtn = document.querySelector(
        ".difficulty-btn.active",
      );

      const questionCount = questionCountEl ? questionCountEl.value : "";
      const examType = examTypeEl ? examTypeEl.value : "";
      const difficulty = activeDifficultyBtn
        ? activeDifficultyBtn.dataset.difficulty
        : "";
